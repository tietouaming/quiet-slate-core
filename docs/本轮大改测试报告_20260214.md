...
- 变更：
  - 由 `relu(abs(tau_tw)-crss)` 改为 `relu(tau_tw-crss)`；
  - 新增 `_twin_nucleation_activation()`，仅有利剪切方向触发形核。

### 6.2 形核源项命名与兼容
- 文件：`mg_coupled_pf/config.py`、`configs/notch_case.yaml`
- 变更：
  - 新参数：`twinning.nucleation_source_amp`；
  - 兼容旧参数：`twinning.langevin_nucleation_noise`（仅作兼容读取）。

### 6.3 surrogate-CP 状态闭合修正
...
- 变更：
  ...
  - surrogate 默认不再直接更新塑性历史场；
  - 若启用塑性场 surrogate 更新，accept 后同步推进 `cp_state.g/gamma_accum`。

### 6.4 死参数处理与配置兼容
- 文件：`mg_coupled_pf/config.py`
- 变更：
  - 删除 `TwinningConfig.h_power`；
  - `load_config()` 增加 dataclass 字段过滤，旧配置多余键不会导致加载失败。
